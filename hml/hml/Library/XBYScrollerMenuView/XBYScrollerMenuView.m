...
- (CGFloat)contentWidthAndAddNavTabBarItemsWithButtonsWidth:(NSArray *)widths
{
    ...
    {
        XBYScrollerMenuTitleCell *cell = [[XBYScrollerMenuTitleCell alloc] initWithFrame:CGRectMake(buttonX, 0, [widths[index] floatValue], self.bounds.size.height) imageView:self.myImageUrlArray[index] title:self.myTitleArray[index]];
...
- (void)setCurrentIndex:(NSInteger)currentIndex
{
    if (_items.count>0) {
        ...
        XBYScrollerMenuTitleCell *cell = (XBYScrollerMenuTitleCell *)_items[currentIndex];
        cell.titleLabel.textColor = self.selectedColor;
        CGFloat offsetX = cell.center.x - self.navigationTabBarWidth * 0.5;
...
//            _line.frame = CGRectMake(cell.frame.origin.x + 2.0f, _line.frame.origin.y, [_itemsWidth[currentIndex] floatValue] - 4.0f, _line.frame.size.height);
//        }];
    }
}
...
-(void)curSelectedIndex:(NSInteger)index
{
    ...
    for (int i=0; i<_items.count; i++) {
        XBYScrollerMenuTitleCell *cell = (XBYScrollerMenuTitleCell *)_items[i];
        cell.titleLabel.textColor = (i==index)?self.selectedColor:self.noSlectedColor;
